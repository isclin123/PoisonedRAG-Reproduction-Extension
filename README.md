...
## 🧠 Phase 1 Requirement Check & Research Outlook
...
| Reproduce the basic PoisonedRAG pipeline | Completed with local substitutions | The pipeline was adapted to local Ollama LLMs rather than the exact original paper LLMs. |
| Explore modified poisoned documents | Completed for Phase 1 | `authority`, `instruction-aware`, and `query-first` variants were tested. |
| Test 1%, 3%, 5%, and 10% poison rates | Completed | The candidate-pool matrix covers all four requested rates. |
| Prepare a GitHub repository | Completed | This repository contains the Phase 1 code, result tables, figures, and reports. |
| Prepare a slide deck | Separate deliverable | Presentation materials should be finalized separately from this code/results repository. |

### 2. Improved poison-document methods

The advisor specifically suggested natural instruction-aware or authority-style poisoned documents. Phase 1 tested those ideas and added a query-first variant.

| Variant | Tested? | Main observation |
| --- | --- | --- |
| `authority` | Yes | Effective, but not consistently stronger than the original poison document. |
| `instruction-aware` | Yes | Slightly stronger than `authority` on average in the candidate-pool setting. |
| `query-first` | Yes | Best average ASR in the candidate-pool setting. |
| `original` | Yes | Strongest in the corpus-level true-ASR setting. |

The main lesson is that better poisoned documents should optimize both retrieval exposure and generation-side credibility. More instruction-like wording alone does not guarantee stronger attacks.

### 3. Statistical roadmap

Because this project studies low-rate attacks, it naturally fits a statistical framing. A statistics-oriented continuation could treat poison rate as a dose and ASR as a response.

Promising statistical extensions include:

- estimating confidence intervals for ASR, retrieval success rate, and correct-answer rate;
- fitting dose-response curves for poison rate versus ASR;
- estimating the minimum poison rate needed to reach 50%, 70%, or 90% ASR;
- using hierarchical logistic regression to separate dataset, model, and poison-document effects;
- adding power analysis to decide how many queries are needed for reliable comparisons;
- testing whether improved poison variants significantly outperform the original variant;
- modeling top-k poison exposure as an intermediate variable between corpus poisoning and final ASR.

This is where a statistics background can become a real research advantage: the project can move from reporting point estimates to estimating uncertainty, thresholds, and risk curves.

### 4. Research extensibility

This direction has strong follow-up potential. Phase 1 already shows that low global poison rates can still matter when poisoned documents are highly query-targeted and likely to enter top-k retrieval.

Natural next steps include:

- moving from "does the attack work?" to "under what statistical conditions does it work?";
- evaluating more realistic attacker knowledge, where the attacker does not know every target query;
- testing stronger retrieval systems such as BGE, E5, hybrid retrieval, and rerankers;
- adding defense experiments such as source filtering, conflict detection, near-duplicate detection, clustering, and evidence-consistency checks;
- comparing model families to understand why some LLMs rely more heavily on poisoned retrieved evidence.

### 5. Open research questions and challenges

Several research questions remain open:

- How low can the true corpus-level poison rate be while still producing statistically reliable ASR?
- Can an attacker succeed without knowing the exact target queries?
- Which matters more: poison-document count, retrieval rank, semantic similarity, or generation-side wording?
- Can rerankers reduce poisoning success, or can adaptive poison documents bypass them?
- How should defenses balance safety with retrieval quality?
- Can poisoned-document traceback identify which retrieved evidence caused a corrupted answer?
- How do these attacks change in multi-hop, agentic, or multimodal RAG systems?

### 6. Current limitations

Phase 1 is a strong initial reproduction and exploration, but it still has important limitations:

- The response LLMs are local Ollama models, not the exact original paper LLMs.
- Most experiment cells use 50-query samples, which are useful for preliminary trends but not enough for strong statistical claims.
- Results are mostly point estimates; confidence intervals and repeated random seeds should be added.
- Corpus-level true-ASR experiments currently compare fewer poison-document variants than the candidate-pool experiments.
- Defense experiments have not yet been run.
- The current threat model is still query-targeted and therefore stronger than many real-world attacker settings.

Overall, Phase 1 establishes a working reproduction pipeline and a meaningful first set of low-rate poisoning results. The next research step should be to make the claims more statistically rigorous and the threat model more realistic.
